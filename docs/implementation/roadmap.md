...
| --- | --- | --- |
| 1. Application and SQLite foundation | Versioned migrations, settings, lifespan, canonical repositories, FTS capability, stable errors/health | Clean temporary install and migration evidence; no user data paths in tests |
| 2. Capture and snapshot lifecycle | URL validation/dedup, durable bookmark-first capture, managed snapshots, retry/restart | Capture/failure/recapture/SSRF scenarios satisfy `CAP-*` |
| 3. Organization and dashboard | Collections, tags, notes, status, detail/list context, responsive states | `ORG-*` and core `UX-*` browser/API evidence |
| 4. Keyword retrieval | Current-version FTS, shared filters, excerpts, rebuild | `RET-01`, `RET-02`, `RET-04` and keyword evaluation pass |
| 5. Provider and semantic indexing | Secret/config/consent boundary, compatible embedding client, index revision, durable jobs | No-transfer-before-consent and configured real-provider smoke pass |
...
| 8. DuckDB analytics | Refresh/checkpoint, shared-filter metrics, stale detection, SQLite fallback | `ANA-*` parity/interruption evidence pass |
...
| 10. Test, package, and release | Complete CI, clean setup/upgrade/rollback, UI smoke, benchmark and docs alignment | Every MVP acceptance item verified or explicitly waived |

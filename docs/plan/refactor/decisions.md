...
| `REF-014` | accepted | The `open-search` command opens InfoBoard as the browser's extension action popup, anchored in the browser UI; `close-search` dismisses it by keyboard shortcut. Neither command uses a detached OS window, side panel, or injected page overlay. | Matches the approved preview while preserving the active tab and requiring no page injection or page-data access; `action` and `commands` are manifest keys, not permissions. | Chrome/Edge action-popup API cannot meet the 480×600, focus, keyboard-open, or keyboard-close acceptance criteria. |
## Rejected shortcuts

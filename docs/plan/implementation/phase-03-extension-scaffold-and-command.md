...
- Ngoài phạm vi:
  ...
  - Không xin hoặc sử dụng quyền history, bookmarks, downloads, cookies, page-content access, scripting/injection, broad host access, network access hay bất kỳ browser database nào. Không đọc DOM của page, không fetch URL, không tạo loopback server và không thêm semantic inference.
...
- [ ] **Manifest và package boundary:** tạo `extension/manifest.json` với `manifest_version: 3`, một `background.service_worker`, surface resource trong extension package và command name/description ổn định. Chọn `action`/icon metadata chỉ khi build target yêu cầu; không thêm product entrypoint thứ hai. Xác định permission allowlist bằng API probe, ưu tiên `tabs`, `windows`, `tabGroups`, `nativeMessaging`; `commands` là manifest key chứ không phải permission.
- [ ] **Permission fail-closed:** viết manifest parser test đọc artifact build, assert command tồn tại đúng một lần, `permissions` là subset allowlist được duyệt, `host_permissions` absent/empty, và reject `history`, `bookmarks`, `downloads`, `cookies`, `scripting`, `activeTab`, `webNavigation`, page-content capability, arbitrary URL patterns hoặc broad host access. Nếu API probe chứng minh một quyền không cần, loại nó khỏi manifest thay vì giữ cho tiện.
- [ ] **Browser adapter contract:** định nghĩa typed methods cho `extensionUrl(path)`, `listSearchSurfaces()`, `createSearchSurface(url, bounds)`, `focusSearchSurface(windowId, tabId)`, `closeSearchSurface(windowId)`, `currentProfileContext()` và lifecycle signal registration. Mỗi method phải trả về bounded result/error, không expose raw `chrome` object; fake adapter phải ghi observable calls để test không cần browser thật.
- [ ] **Command dispatch:** đăng ký một top-level `commands.onCommand` listener cho `open-search`; kiểm tra command name, lấy profile context từ adapter, tìm surface thuộc đúng extension URL/profile, rồi focus existing surface hoặc tạo focused popup surface. Dedupe concurrent invocation bằng operation token và always settle/release it on success, API error, cancellation hoặc worker suspension.
- [ ] **Focused surface creation:** tạo surface bằng extension-owned URL, kích thước nhỏ hợp lý và `focused: true`; không navigate current tab. Surface route phải có `noopener`-equivalent browser boundary tự nhiên (extension page), không load remote script/style/font và không nhận URL tùy ý từ command input.
- [ ] **Keyboard focus handoff:** trong `surface-entrypoint.ts`, tạo fresh session, reset query value, render accessible shell, chờ DOM ready rồi gọi focus query input với `preventScroll`; assert `document.activeElement === queryInput` trong fixture trước khi dispatch text. Focus failure, missing input hoặc closed document phải phát signal lỗi recoverable, không giả vờ đã sẵn sàng.
- [ ] **Dismissal boundary:** wire `Esc` và explicit close action tới adapter close surface; dismissal không gọi tab activation, không đổi selected tab/window và không lưu query/title/URL vào log. IP-13 có thể mở rộng keyboard navigation nhưng không được thay đổi close invariant.
- [ ] **Service-worker lifecycle:** đăng ký listeners synchronously at module evaluation, giữ worker stateless ngoài bounded in-flight operation map, xử lý `runtime.onStartup`/`onInstalled` để validate package/adapter readiness, và recreate adapter after suspension. Không dùng polling, websocket, loopback server, long-lived timer hay page-content listener để giữ worker sống.
- [ ] **Browser parity:** implement namespace resolution/configuration cho Chrome và Edge cùng contract; fixture phải chạy cùng assertions với `BROWSER=chrome` và `BROWSER=edge`, chỉ thay binary/namespace. API divergence phải được normalize tại adapter, không rẽ nhánh trong command handler/surface.
- [ ] **Handoff to later phases:** expose surface-ready/session/profile context without owning query protocol; expose adapter events for IP-04/IP-14 without implementing projection or stale-result activation. Ghi rõ invariant rằng extension browser authority quyết định window/tab activation và host không được mở surface.
- [ ] **Targeted tests:** fixture phải cover command success, existing surface, duplicate invocation, missing/closed surface, permission denial/API rejection, worker restart, focus-before-text, missing query input, Esc dismissal, and no network/page read. Tests assert observable browser state and emitted status, not implementation details.

## 7. Kế hoạch commit

1. `feat(extension): add minimal Manifest V3 command scaffold`
   - Thay đổi: tạo `extension/manifest.json`, service-worker entry boundary và package resource declarations; giữ permission allowlist tối thiểu, không host patterns.
   - Cách kiểm tra: parse built manifest with the permission fixture; assert exactly one `open-search` command and a service-worker entry for both Chrome and Edge build targets.
2. `feat(extension): add browser adapter and focused surface handoff`
   - Thay đổi: tạo adapter seam, command dispatch, create-or-focus surface logic, fresh session and query-input focus handoff, Esc dismissal boundary.
   - Cách kiểm tra: run command fixture with fake adapter and browser harness; observe extension surface URL, focused window/input, no selected-tab mutation, and bounded error on API rejection.
3. `feat(extension): make service worker restart safe`
   - Thay đổi: top-level listener registration, startup/install readiness, suspension/resume reconstruction and cancellation cleanup; remove any worker-global durable state.
   - Cách kiểm tra: suspend worker between command events, replay command, and verify the same create/focus/focus-input result without duplicate surface or leaked operation.
4. `test(extension): cover command permissions and keyboard focus contract`
   - Thay đổi: add `manifest-permissions.test.ts`, `command-open-surface.test.ts`, `service-worker-lifecycle.test.ts` and `command-open-surface.json` fixture.
   - Cách kiểm tra: run the exact Chrome and Edge targeted commands in Section 8; require observable focus and permission rejection assertions, not source snapshots.
...
- [ ] Parse the built Manifest V3 artifact in the permission fixture. Acceptance requires exactly one command, only the reviewed minimum permission subset, no `host_permissions` patterns, and explicit absence of `history`, `bookmarks`, `downloads`, cookies, page-content/injection capability, and broad host access. A newly added permission must fail the fixture until its decision/contract is updated.

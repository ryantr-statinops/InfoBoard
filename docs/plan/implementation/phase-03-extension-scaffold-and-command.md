...
- Bao gồm:
  ...
  - Permissions at this phase are exactly the APIs currently used: `storage`, `tabs`, and `windows`. Storage may access only REF-013's `chrome.storage.local` key `profile_id`; no host patterns are declared and `host_permissions` is absent. Defer `tabGroups` to IP-04 and `nativeMessaging` to IP-07; `commands` is a manifest key, not a permission.
...
- [ ] `IP-03-T01` **Manifest và package boundary:** tạo `extension/manifest.json` với `manifest_version: 3`, một `background.service_worker`, surface resource trong extension package và command name/description ổn định. Chọn `action`/icon metadata chỉ khi build target yêu cầu; không thêm product entrypoint thứ hai. Phase-local permission allowlist is exactly `storage`, `tabs`, and `windows`; storage is only REF-013's identity key, with no host access. Defer `tabGroups` to IP-04 and `nativeMessaging` to IP-07; `commands` is a manifest key, not a permission.
...
1. `feat(extension): implement ip-03-t01`
   ...
   - Behavior: **Manifest và package boundary:** tạo `extension/manifest.json` với `manifest_version: 3`, a module service worker, one search-shell resource, and one `open-search` command. Phase-local permissions are exactly `storage`, `tabs`, and `windows`; the storage key is limited to REF-013. Defer `tabGroups` and `nativeMessaging` to their owner phases; `commands` is a manifest key, not a permission.
   ...
   - Observable result before commit: the built manifest has only the three phase-local permissions, exactly one configurable command, no host permissions, and packaged service-worker/search resources.
...
- [ ] From repository root, run `npm ci`, `npm run build:extension`, then `BROWSER=chrome npm run test:extension -- tests/extension/manifest-permissions.test.ts tests/extension/command-open-surface.test.ts tests/extension/service-worker-lifecycle.test.ts`. The command loads `fixtures/extension/command-open-surface.json` directly. The suite must report the manifest allowlist, command journey, lifecycle, focus and dismissal assertions as passing.
- [ ] Repeat with `BROWSER=edge npm run test:extension -- tests/extension/manifest-permissions.test.ts tests/extension/command-open-surface.test.ts tests/extension/service-worker-lifecycle.test.ts`. The runner must use the declared Edge binary and fail if the adapter depends on Chrome-only behavior.
- [ ] In each browser fixture, invoke configured `open-search` without pointer input. Observe the extension-owned surface for the active context, exactly one surface after two rapid invocations, and `document.activeElement.id === "query"` before typing; the first character is accepted by that input.

...
- [ ] **Việc 2 — Implement the permission matrix and static audit.** Keep the manifest allowlist narrow and map each capability as follows:
  ...
  | Extension local storage API | Not required; SQLite owns declared local persistence through the host. | MUST NOT be requested or called. | No fallback to browser local storage; use in-memory defaults and report persistence degradation. | Static API scan and excluded-surface fixture. |

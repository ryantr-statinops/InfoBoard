...
- Bao gồm:
  ...
  - Static permission inspection and runtime spies/fixtures prove there is no network request, page evaluation/read, browser-history access, cookie/session access, page local/session-storage access, use of `storage.sync`, broad host access, or external telemetry in the normal path. REF-013 permits only the extension-owned `chrome.storage.local` key `profile_id`. The Go host uses only the registered framed Native Messaging transport.
...
- Ngoài phạm vi:
  ...
  - Whole-browser history, saved-item/download data, page DOM or rendered text, selected text, cookies, page `window.localStorage`/session data, `storage.sync`, network interception, cloud services, account sync, cross-profile search, unsupported browsers, mobile, and any remote or loopback transport. REF-013's identity-only extension storage is the sole exception.

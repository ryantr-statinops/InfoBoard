...
- [ ] **Bootstrap and process boundary:** create the thin `main` entrypoint and runtime configuration. Acquire stdin/stdout once, reject unsupported invocation/configuration before accepting tab data, route diagnostics to a bounded redacted sink, and make process exit codes distinguish clean close from startup/protocol/runtime failure without leaking sensitive input.
- [ ] **Framed stream ownership:** implement a transport seam in which exactly one reader owns stdin and exactly one serialized writer owns stdout. Enforce the protocol frame size before allocation (with IP-07), flush complete frames, reject partial/malformed input fail-closed, and ensure no `fmt.Print`, default logger, panic text, or child-process output can contaminate stdout.
- [ ] **Session state machine:** implement legal transitions and idempotent terminal behavior. Do not mark `healthy` at process start or after handshake alone; require protocol compatibility and successful projection/index readiness handoff. A session with unknown profile, invalid version, failed limits, or rebuilding projection remains unavailable/recovering and cannot accept query work.
- [ ] **Startup handoff:** order runtime limit construction, protocol hello/ack, profile validation, persistence readiness callback, and snapshot/index readiness callback. Each handoff receives a context/deadline and returns a typed safe error. Do not send or persist tab data before protocol/profile checks succeed.
- [ ] **Normal shutdown:** on explicit close or clean stdin EOF, stop accepting new work, cancel request contexts, wait only up to the configured shutdown budget, flush bounded local metadata through the IP-08 interface if available, emit a final safe status/error when writable, close stdout and dependencies exactly once, and exit without closing tabs or touching unrelated browser state.
- [ ] **Failure and reconnect:** classify broken pipe, EOF, host crash, context cancellation, timeout, protocol mismatch, and dependency failure separately. A session failure must be visible to the extension as unavailable or recovering with retryability. Reconnect means the extension requests a new browser Native Messaging connection; the new host session starts clean and requires an authoritative snapshot, rather than trusting stale memory or trying to connect to a port.
- [ ] **Cancellation and bounded concurrency:** derive per-request deadlines from one central budget, propagate cancellation to protocol/index/persistence callbacks, cap in-flight operations and queued frames, and make blocked fake transport operations interruptible in tests. On deadline, return the relevant safe class (for example `QUERY_TIMEOUT` or `HOST_SHUTDOWN` at the owning wire layer) and release all per-request resources.
- [ ] **Health and diagnostics:** expose `healthy`, `unavailable`, and `recovering` transitions to an observer consumed by IP-07/IP-16. Record only state, cause/error class, retryability, duration, counts, versions, and projection revision/count when allowed. Redact raw tab fields and keep diagnostics bounded; persistence failure changes the supplied persistence status but does not make lexical runtime unavailable by itself.
- [ ] **Resource and security limits:** enforce configured maximum frame/queue/request sizes, concurrent work, startup and shutdown durations, and diagnostic writes before large allocations or goroutine creation. Avoid network APIs, filesystem writes outside explicit lifecycle dependencies, shell execution, and process spawning in the hot path. Verify that no package opens a listening socket or dials a remote endpoint.
- [ ] **Fixture-driven tests:** implement the eight host fixtures above with a deterministic clock and fake `io.Reader`/`io.Writer`/transport. Assert observable state/output, exact bounded completion, frame integrity, error class/retryability, and cleanup; do not assert private goroutine layout or incidental log text.

## 7. Kế hoạch commit

1. `feat(host): bootstrap native messaging lifecycle`
   - Thay đổi: add the Go entrypoint, runtime/session state machine, stdin/stdout ownership, lifecycle transitions, health observer, and dependency handoff interfaces under the owned `host/` paths.
   - Cách kiểm tra: `go test ./host/... -run 'Test(Healthy|Unavailable|Recovering|StdioOwnership|Lifecycle)'` from repository root with `fixtures/host/phase-06/` fixture inputs; verify stdout contains framed messages only.
2. `feat(host): bound cancellation and runtime resources`
   - Thay đổi: add central timeout/resource budgets, cancellation causes, bounded shutdown/reconnect behavior, safe diagnostics, and fail-closed invalid/disconnected stream handling.
   - Cách kiểm tra: `go test ./tests/host/phase-06 -run 'Test(Timeout|Cancel|Disconnect|ResourceLimit|NoLoopback)' -count=1`; assert every fixture completes within its declared deadline and no network listener/dial is observed.
3. `test(host): cover lifecycle failure recovery`
   - Thay đổi: add deterministic fake-transport and process/session fixtures for startup failure, EOF/crash, fresh-session snapshot recovery, serialization, and persistence-degraded handoff.
   - Cách kiểm tra: `go test ./host/... ./tests/host/phase-06 -count=1`; run the future race command `go test -race ./host/... ./tests/host/phase-06` and retain transition evidence for NFR-006/FR-012.

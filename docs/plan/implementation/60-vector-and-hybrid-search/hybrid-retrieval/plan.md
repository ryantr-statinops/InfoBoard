# Semantic and hybrid retrieval: implementation plan

## Approach

Orchestrate semantic query embeddings, vector candidates, shared filters, deterministic fusion, deduplication, and keyword fallback. Implement the boundary in search orchestrator, fusion/ranking module, `/api/v1/search`, UI mode controls, relevance tests. Use explicit adapters so provider and store failure cannot leak into canonical repositories or keyword retrieval.
...
Complete RET-KEYWORD, SEM-CONFIG, VEC-CHROMA; deliver [tasks](tasks.md) in order; record real [evidence](execution.md); update rollups only after milestone acceptance.

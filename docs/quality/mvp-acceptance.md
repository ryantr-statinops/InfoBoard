...
- Preserve integer IDs and existing text/file content through migration; legacy items remain readable, searchable, organizable, and soft-deletable without capture controls.
- Organize with multiple collections/tags, notes, and status without index coupling.
- Reject missing/stale mutation preconditions without overwriting newer bookmark, membership, collection, tag, or note state.
...
- Time series enforce the 30-day default/366-day maximum; totals/top lists enforce deterministic top-20 output through both analytics backends.
...
- Canonical `/api/v1` routes and target-compatible `/api/*` adapters return equivalent behavior for one release; retired text/file writes return documented `410` errors and all adapters emit deprecation metadata.
